...
namespace pso_planner
{
  ...
  public:
     ...
     */
    // PSOplanner(int nx, int ny, double resolution, double origin_x, double origin_y, int n_particles,int n_inherited, int pointNum , double w_inertial, double w_social, double w_cognitive, double obs_factor,int max_speed,int initposmode ,bool pub_particles,int max_iter);
    // ~PSOplanner();
      PSOplanner();
      ~PSOplanner();
    /**
     ...
     */
    bool plan(costmap_2d::Costmap2D* global_costmap, const std::pair<int, int>& start, const std::pair<int, int>& goal, std::vector< std::pair<int, int>>& path);
