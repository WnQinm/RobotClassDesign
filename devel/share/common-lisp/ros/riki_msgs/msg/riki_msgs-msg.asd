
(cl:in-package :asdf)

(defsystem "riki_msgs-msg"
  :depends-on (:roslisp-msg-protocol :roslisp-utils :geometry_msgs-msg
)
  :components ((:file "_package")
    (:file "Arm" :depends-on ("_package_Arm"))
    (:file "_package_Arm" :depends-on ("_package"))
    (:file "Battery" :depends-on ("_package_Battery"))
    (:file "_package_Battery" :depends-on ("_package"))
    (:file "Blue_connect" :depends-on ("_package_Blue_connect"))
    (:file "_package_Blue_connect" :depends-on ("_package"))
    (:file "Bluetooth" :depends-on ("_package_Bluetooth"))
    (:file "_package_Bluetooth" :depends-on ("_package"))
    (:file "Buzzer" :depends-on ("_package_Buzzer"))
    (:file "_package_Buzzer" :depends-on ("_package"))
    (:file "DHT22" :depends-on ("_package_DHT22"))
    (:file "_package_DHT22" :depends-on ("_package"))
    (:file "Imu" :depends-on ("_package_Imu"))
    (:file "_package_Imu" :depends-on ("_package"))
    (:file "Infrared" :depends-on ("_package_Infrared"))
    (:file "_package_Infrared" :depends-on ("_package"))
    (:file "Led" :depends-on ("_package_Led"))
    (:file "_package_Led" :depends-on ("_package"))
    (:file "PID" :depends-on ("_package_PID"))
    (:file "_package_PID" :depends-on ("_package"))
    (:file "RcMode" :depends-on ("_package_RcMode"))
    (:file "_package_RcMode" :depends-on ("_package"))
    (:file "Servo" :depends-on ("_package_Servo"))
    (:file "_package_Servo" :depends-on ("_package"))
    (:file "Sonar" :depends-on ("_package_Sonar"))
    (:file "_package_Sonar" :depends-on ("_package"))
    (:file "Ultrasonic" :depends-on ("_package_Ultrasonic"))
    (:file "_package_Ultrasonic" :depends-on ("_package"))
    (:file "Velocities" :depends-on ("_package_Velocities"))
    (:file "_package_Velocities" :depends-on ("_package"))
    (:file "ps2_value" :depends-on ("_package_ps2_value"))
    (:file "_package_ps2_value" :depends-on ("_package"))
  ))